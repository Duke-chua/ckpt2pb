...
def main(_):
  ...
  with tf.Graph().as_default():
    ...
    def clone_fn(batch_queue):
      ...
      with tf.variable_scope("InceptionV3", reuse=True) as scope:
          ...
          with slim.arg_scope([slim.conv2d, slim.max_pool2d, slim.avg_pool2d],
              ...
              with tf.variable_scope("AuxLogits",reuse=tf.AUTO_REUSE):
                  ...
                  print('aux_logits')
                  print(aux_logits)
                  aux_logits = slim.conv2d(aux_logits, num_outputs=2, kernel_size=[1, 1],
                                           activation_fn=None, normalizer_fn=None,
                                           weights_initializer=trunc_normal(0.001), scope="Conv2d_10b_1x1")
                  # 消除tensor中前两个维度为1的维度
      ...
      with tf.name_scope('output'):
          ...
          hinge_loss = tf.reduce_mean(
              tf.square(
                  tf.maximum(
                      #tf.zeros([16, 2]), 1 - (labels*2-1.0) * logits
                      tf.zeros([32, 2]), 1 - (labels * 2 - 1) * logits2
                  )
              )
          )
          # with tf.name_scope("loss"):
          loss = 0.5*regularization_loss + 1.0* hinge_loss
          print('loss')
          print(loss)
    ...
    with tf.name_scope('evaluation'):
        correct_prediction = tf.equal(tf.argmax(end_points["final_tensor"], 1), tf.argmax(end_points["labels"], 1))
